<?php
    ...
    if(isset($_POST["consumption"]) && isset($_POST["resourceid"]) && isset($_POST["amount"]) && isset($_POST["unit"]) && isset($_POST["used"]) && isset($_POST["remaining"]) && isset($_POST["dateused"]) )
    {
      $consumption_v = validate($_POST["consumption"], $conn);
      $resourceid_v = validate($_POST["resourceid"], $conn);
      $amount_v = validate($_POST["amount"], $conn);
      $unit_v = validate($_POST["unit"], $conn);
      $used_v = validate($_POST["used"], $conn);
      $remaining_v = validate($_POST["remaining"], $conn);
      $dateused_v = validate($_POST["dateused"], $conn);

      $sql = "SELECT consumption_id FROM consumption_t WHERE consumption_id = {$_POST['consumption']}";  
      $result = mysqli_query($conn, $sql);
        
      if(mysqli_num_rows($result) > 0){
        echo "<div class='alert alert-danger' role='alert'>Record with the same Consumption ID exists.</div>";
      }
      else{
        // insert into egg_reading_t
        $sql = "INSERT INTO consumption_t VALUES('$consumption_v','$resourceid_v', '$amount_v', '$unit_v', '$used_v', '$remaining_v', '$dateused_v')";  
        $result = mysqli_query($conn, $sql);

        // insert activity to activity_t
        $activity = "Add new Resource consumption values = ({$consumption_v},{$resourceid_v}, {$amount_v}, {$unit_v}, {$used_v}, {$remaining_v}, {$dateused_v})";
        $id = $_SESSION["id_s"];
  ...
  <div class="content-wrapper bg-custom1">
    <div class="content-header bg-custom1">
      <div class="container-fluid text-white">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Add Consumption</h1>
          </div>
          <div class="col-sm-6">
      ...
      <div class="container-fluid">
        <div class="col">
          <div class="card bg-custom text-white">
            ...
            <div class="card-body">
              <form class="row rounded" action="add-consumption.php" method="post">
                  <div class="col-md-6">
                      <div class="mb-3">
                          <label for="consumption" class="form-label">Consumption ID</label>
                          <input type="text" name="consumption" class="form-control" required>
                      </div>
                      <div class="mb-3">
                          <label for="used" class="form-label">Used</label>
                          <input type="number" name="used" class="form-control" min='1' required>
                      </div>
                      <div class="mb-3">
                  ...
                  <div class="col-md-6">
                      ...
                      <div class="mb-3">
                          ...
                          <input type="number" name="remaining" class="form-control" min='1' required>
                      </div>
                      <div class="mb-3">
                          <label for="dateused" class="form-label">Date Used </label>
                          <input type="datetime-local" name="dateused" class="form-control" required>
                      </div>
                      <br>
                      <br>
