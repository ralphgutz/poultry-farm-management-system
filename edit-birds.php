<?php
    ...
    function get_old_values($id, $conn) {
        // getting old values
        $sql = "SELECT * FROM birds_t WHERE batch_id = '$id'";
        $query = mysqli_query($conn, $sql);
        $row_data = mysqli_fetch_assoc($query);

        $batch_id_old = $row_data["batch_id"];
        $bird_breed_old = $row_data["bird_breed"];
        $quantity_old = $row_data["quantity"];
        $update_date_old = $row_data["update_date"];

        return array($id, $batch_id_old, $bird_breed_old,$quantity_old, $update_date_old);
    }
    ...
    if(isset($_GET["delete"])){  // for delete
      $id = $_GET["delete"];  
          ...
          list($id, $batch_id_old, $bird_breed_old,$quantity_old, $update_date_old) = get_old_values($id, $conn);

          // add update activity to activity_t
          $activity = "Delete batch_id = {$id}. Values = ({$bird_breed_old}, {$quantity_old}, {$update_date_old})";
          $id = $_SESSION["id_s"];
        ...
        $sql = "DELETE FROM birds_t WHERE batch_id= $id_d";
      ...
      elseif(isset($_POST["update"])){ 
        $id = $_POST["batch_id"]; //updating data

        $batch_id = $_POST["batch_id"];
        $bird_breed = $_POST["bird_breed"];
        $quantity = $_POST["quantity"];
        date_default_timezone_set("Asia/Manila");
        $update_date = (new DateTime('now'))->format('Y-m-d H:i:s');

        // getting old values
        list($id, $batch_id_old, $bird_breed_old,$quantity_old, $update_date_old) = get_old_values($id, $conn);

        // updating data into birds_t table
        $sql = "UPDATE birds_t SET bird_breed = '$bird_breed', quantity = '$quantity', update_date = '$update_date' WHERE batch_id = '$id'";
        $query = mysqli_query($conn, $sql);

        // add update activity to activity_t
        $activity = "Update batch_id = {$id}. Old values = ({$bird_breed_old}, {$quantity_old}, {$update_date_old}). New values = ({$bird_breed}, {$quantity}, {$update_date})";
        $id = $_SESSION["id_s"];
        date_default_timezone_set("Asia/Manila");
        $datetime = date("Y-m-d H:i:s");
        $sql = "INSERT INTO activity_t(emp_id, activity, act_datetime) VALUES ('$id', '$activity', '$datetime')";  
  ...
  <div class="content-wrapper bg-custom1">
    <div class="content-header bg-custom1">
      <div class="container-fluid text-white">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Edit/Delete Birds</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Edit Birds</li>
            </ol>
          </div>
        </div>
      </div>
        ...
        <div class="col">
          <div class="card bg-custom text-white">
            <div class="card-header bg-custom2 border-0">
              <h3 class="card-title">Birds Table</h3>
            </div>
            <div class="card-body">
              <table id="example1" class="table table-striped table-hover">
                ...
                <?php
                  ...
                  while($row_data = mysqli_fetch_array($query)){
                    ...
                    echo "<td><a href='edit-birds.php?edit={$row_data['batch_id']}' class='btn btn-warning'><i class='fas fa-edit'></i></a>
                              <a href='edit-birds.php?delete={$row_data['batch_id']}' class='btn btn-danger' onclick='return confirm(`Are you sure you want to delete this record?`);'><i class='fas fa-trash-alt'></i></a>
                          </td>";
                    echo "</tr>";
                  }
                ?>
      ...
      <div class="container-fluid">
        <div class="col">
          <div class="card bg-custom text-white">
            ...
            <div class="card-body">
              <form class="row rounded" action="edit-birds.php" method="post">
                  <div class="col-md-6">
                      <input type="hidden" name="id">
                      <div class="mb-3">
                          <label for="batch_id" class="form-label">Batch ID</label>
                          <input type="text" name="batch_id" class="form-control" value="<?php echo $batch_id?>" readonly>

                      </div>
                      <div class="mb-3">
                          <label for="bird_breed" class="form-label">Bird Breed</label>
                          <input type="text" name="bird_breed" class="form-control" value="<?php echo $bird_breed?>" required>
                      </div>
                  </div>
                  ...
                  <div class="col-md-6">
                      <div class="mb-3">
                          ...
                          <input type="number" name="quantity" class="form-control" min="1" value="<?php echo $quantity?>" min="1"  required>
                      </div>
